...
import HinaSkillConfig from './skills/HinaSkillConfig';
...
type TabType = 'general' | 'model' | 'coworkSandbox' | 'coworkMemory' | 'shortcuts' | 'im' | 'email' | 'hina' | 'about';
...
const Settings: React.FC<SettingsProps> = ({ onClose, initialTab, notice }) => {
  ...
  const sidebarTabs: { key: TabType; label: string; icon: React.ReactNode }[] = useMemo(() => [
    ...
    { key: 'hina',           label: i18nService.t('hinaTab'),        icon: <EnvelopeIcon className="h-5 w-5" /> },
